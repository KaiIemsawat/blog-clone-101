...
export const updateUser = async (req, res, next) => {
    if (req.user.id !== req.params.userId) {
        return next(
            errorHandler(403, "You are not allowed to update this user details")
        );
    }
...
export const getUsers = async (req, res, next) => {
    if (!req.user.isAdmin) {
        return next(
            errorHandler(403, "You don't have permission to see all users")
        );
    }
    try {
        ...
        const usersWithotPassword = users.map((user) => {
